...
argument-hint: [bootstrap | verify | cleanup | reap | install-reaper | full]
---
...
- `reap` — delete any leftover scratch repos in the hardcoded `REAP_ALLOWLIST` that are older than `--ttl-hours` (default 48). The backstop for when `cleanup` isn't run, so scratch repos never leak but you still get an observation window. Uses your own `gh` creds (no delete-scoped secret). Supports `--dry-run`. Can only ever touch the two allowlisted names, never a real repo.
- `install-reaper` — one-time setup (macOS): renders the `scripts/genesis-reap.plist` template with *this clone's* absolute path, writes it to `~/Library/LaunchAgents/ai.sayfan.genesis-reap.plist`, and loads it so `reap` runs hourly. This is how a fresh clone stands the reaper up; the plist ships as a path-agnostic template, so don't copy it by hand. On Linux the command prints the equivalent cron/systemd-timer line instead.
